...
pub async fn mk_lib_database_version(pool: &sqlx::PgPool) -> Result<i32, sqlx::Error> {
...
pub async fn mk_lib_database_version_check(
    ...
    if version_match == false {
        if update_schema == true {
            // do db updates here
            mk_lib_database_version_schema::mk_lib_database_update_schema(&pool, version_no)
                .await?;
            version_match = true;
        } else {
            loop {
