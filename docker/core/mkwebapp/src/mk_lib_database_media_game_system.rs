...
pub async fn mk_lib_database_media_game_system_read(pool: &sqlx::PgPool,
    ...
    if search_value != "" {
        let rows = sqlx::query("select gs_game_system_id, \
            gs_game_system_name, \
...
pub async fn mk_lib_database_media_game_system_count(pool: &sqlx::PgPool,
                                                     search_value: String)
                                                     -> Result<i64, sqlx::Error> {
    if search_value != "" {
        let row: (i64, ) = sqlx::query("select count(*) from mm_metadata_game_systems_info where gs_game_system_name = $1")
            .bind(search_value)
            .fetch_one(pool)
            .await?;
        Ok(row.0)
