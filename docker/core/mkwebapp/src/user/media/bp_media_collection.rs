...
pub async fn user_media_collection(sqlx_pool: &rocket::State<sqlx::PgPool>, user: User, page: i8) -> Template {
    let total_pages: i32 = mk_lib_database_metadata_collection::mk_lib_database_metadata_collection_count(&sqlx_pool, "".to_string()).await.unwrap() / 30;
    let pagination_html = mk_lib_common_pagination::mk_lib_common_paginate(total_pages, page).await.unwrap();
    let collection_list = mk_lib_database_metadata_collection::mk_lib_database_metadata_collection_read(&sqlx_pool, "".to_string(), 0 ,30).await.unwrap();
...
pub async fn user_media_collection_detail(sqlx_pool: &rocket::State<sqlx::PgPool>, user: User, guid: Uuid) -> Template {
    Template::render("bss_user/metadata/bss_user_metadata_movie_collection_detail", {})
}
