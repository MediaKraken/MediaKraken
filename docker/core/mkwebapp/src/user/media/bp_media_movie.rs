...
pub async fn user_media_movie(sqlx_pool: &rocket::State<sqlx::PgPool>, user: User, page: i32) -> Template {
    let db_offset: i32 = (page * 30) - 30;
    let mut total_pages: i64 = mk_lib_database_media_movie::mk_lib_database_media_movie_count(&sqlx_pool, String::new()).await.unwrap();
    if total_pages > 0 {
        total_pages = total_pages / 30;
    }
    let pagination_html = mk_lib_common_pagination::mk_lib_common_paginate(total_pages, page, "/user/media/movie".to_string()).await.unwrap();
    let movie_list = mk_lib_database_media_movie::mk_lib_database_media_movie_read(&sqlx_pool, String::new(), db_offset, 30).await.unwrap();
    Template::render("bss_user/media/bss_user_media_movie", &TemplateMediaMovieContext {
...
pub async fn user_media_movie_detail(sqlx_pool: &rocket::State<sqlx::PgPool>,
     user: User, guid: rocket::serde::uuid::Uuid) -> Template {
        ...
        Template::render("bss_user/media/bss_user_media_movie_detail", tera::Context::new().into_json())
}
