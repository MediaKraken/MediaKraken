...
#[get("/media/game/<page>")]
pub async fn user_media_game(sqlx_pool: &rocket::State<sqlx::PgPool>, user: User, page: i32) -> Template {
    let db_offset: i32 = (page * 30) - 30;
    let mut total_pages: i64 = mk_lib_database_media_game::mk_lib_database_media_game_count(&sqlx_pool, String::new()).await.unwrap();
    if total_pages > 0 {
        total_pages = total_pages / 30;
    }
    let pagination_html = mk_lib_common_pagination::mk_lib_common_paginate(total_pages, page, "/user/media/game".to_string()).await.unwrap();
    let game_list = mk_lib_database_media_game::mk_lib_database_media_game_read(&sqlx_pool, String::new(), db_offset, 30).await.unwrap();
