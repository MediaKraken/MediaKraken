            ...
            option_config_json["MAME"]["Version"], option_config_json["MAME"]["Version"]
                        ...
                        if !Path::new(&unzip_file_name).exists() {
                            ...
                            for line in reader.lines() {
                                ...
                                if xml_line.starts_with("<machine") == true {
                                    ...
                                    mk_lib_database::database_metadata::mk_lib_database_metadata_game::mk_lib_database_metadata_game_upsert(
                    &sqlx_pool,
                    uuid::Uuid::nil(),
                ...
                .unwrap();
                    ...
                    if !Path::new(&file_name).exists() {
                        ...
                        .await
                        .unwrap();
                        for zippedfile in mk_lib_file::mk_lib_file::mk_directory_walk(format!(
                        ...
                        .unwrap()
                        {
                            ...
                            if ext == ".xml" {
                                let file = File::open(&zippedfile).unwrap();
                                let reader = BufReader::new(file);
                                let mut xml_data: String = "".to_owned();
                                ...
                                for line in reader.lines() {
                                    ...
                                    if xml_line.starts_with("<softwarelist") == true {
                                        ...
                                        let mut game_system_uuid = mk_lib_database::database_metadata::mk_lib_database_metadata_game_system::mk_lib_database_metadata_game_system_guid_by_short_name(&sqlx_pool, &system_string_split[1].to_string()).await.unwrap();
                                ...
                                Ok(_) => {
                                    if skip_next_line {
                                            ...
                                            || dat_line.starts_with("$mame")
                                        {
                                            ...
                                            if dat_line.rsplit(" ").last().unwrap().trim_end_matches('\n') == "Supported" {
                                                sys_save_state = true;
                                            }
                                            else {
                                            ...
                                            if wip_in_progress && dat_line.find("Romset:") != Some(0) {
                                                sys_wip += format!("{}<BR>", dat_line.trim_end_matches('\n').to_string());
                                            }
                                            if romset_in_progress && dat_line.find("$end") != Some(0) {
                                                sys_romset += format!("{}<BR>", dat_line.trim_end_matches('\n').to_string());
                                            }
                                            if desc_next {
                                                sys_desc = dat_line;
                                                desc_next = false;
                                            ...
                                            if long_name_next {
                                             ...
                                             if dat_line.starts_with("$end") {  // end of system info so store system into db
                                                romset_in_progress = false;
                                                if sys_desc.trim_end_matches('\n') == "..." {
                                                    sys_desc = String::new();
                                                }
                                                else {
                                                    sys_desc = sys_desc.trim_end_matches('\n').to_string();
                                                }
                                                sys_emulation = sys_emulation.trim_end_matches('\n').to_string();
                                                sys_color = sys_color.trim_end_matches('\n').to_string();
                                                sys_sound = sys_sound.trim_end_matches('\n').to_string();
                                                sys_graphics = sys_graphics.trim_end_matches('\n').to_string();
                                            }
                                            // upsert the system
                                            mk_lib_database::database_metadata::mk_lib_database_metadata_game_system::mk_lib_database_metadata_game_system_upsert(&sqlx_pool, 
