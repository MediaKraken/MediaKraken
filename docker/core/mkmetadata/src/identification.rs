...
pub async fn metadata_identification(pool: &sqlx::PgPool,
    ...
    match dl_row["mdq_que_type"] {
        mk_lib_common_enum_media_type::DLMediaType::ADULT => {
            let metadata_uuid = metadata_adult::metadata_adult_lookup(&sqlx_pool,
                                                                      dl_row,
                                                                      guessit_data);
        }

        mk_lib_common_enum_media_type::DLMediaType::ANIME => {
            let metadata_uuid = metadata_anime::metadata_anime_lookup(&sqlx_pool,
                                                                      dl_row,
                                                                      guessit_data);
        }
        ...
        mk_lib_common_enum_media_type::DLMediaType::PUBLICATION_GRAPHIC_NOVEL => {
            let metadata_uuid = metadata_periodicals::metadata_periodicals_lookup(&sqlx_pool,
                                                                                  dl_row);
        }
        ...
        mk_lib_common_enum_media_type::DLMediaType::MOVIE_TRAILER => {
            let metadata_uuid = metadata_movie::metadata_movie_lookup(&sqlx_pool,
                                                                      dl_row,
                                                                      guessit_data);
        }
        ...
        mk_lib_common_enum_media_type::DLMediaType::MUSIC_SONG => {
            let metadata_uuid = metadata_music::metadata_music_lookup(&sqlx_pool,
                                                                      dl_row);
        }

        mk_lib_common_enum_media_type::DLMediaType::MUSIC_VIDEO => {
            let metadata_uuid = metadata_music_video.metadata_music_video_lookup(&sqlx_pool,
                                                                                 dl_row);
        }

        mk_lib_common_enum_media_type::DLMediaType::SPORTS => {
            let metadata_uuid = metadata_sports.metadata_sports_lookup(&sqlx_pool,
                                                                       dl_row);
        }
        ...
        mk_lib_common_enum_media_type::DLMediaType::TV_TRAILER => {
            let metadata_uuid = metadata_tv.metadata_tv_lookup(&sqlx_pool,
                                                               dl_row,
                                                               guessit_data);
        }
...
async def metadata_identification(&sqlx_pool, dl_row, guessit_data):
    ...
    #                                                        guessit_data)
        ...
        metadata_uuid = await metadata_game.metadata_game_lookup(&sqlx_pool,
                                                                 dl_row);
                                                                 }
