...
if [[ ! -e $serverFile ]]; then
    ...
    wget 'https://www.dropbox.com/s/dnjan96ol1t5rms/bf1942_lnxded-1.6-rc2.run?dl=0' -qO $serverFile
fi

# download patch if not already downloaded
if [[ ! -e $patchFile ]]; then
    echo 'downloading 1.61b patch'
    wget 'https://www.dropbox.com/s/9sfg0hd8xae42r5/bf1942-update-1.61.tar.gz?dl=0' -qO $patchFile
fi
