...
if [[ ! -e $serverFile ]]; then
    ...
    wget 'http://fizweb.elte.hu/battlefield/Battlefield-1942/Server-Linux/bf1942_lnxded-1.6-rc2.run' -qO $serverFile
fi

# download patch if not already downloaded
if [[ ! -e $patchFile ]]; then
    echo 'downloading 1.61b patch'
    wget 'http://fizweb.elte.hu/battlefield/Battlefield-1942/Server-Linux/bf1942-update-1.61.tar.gz' -qO $patchFile
fi
