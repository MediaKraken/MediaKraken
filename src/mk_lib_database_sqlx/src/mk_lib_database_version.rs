...
#[allow(dead_code)]
pub async fn mk_lib_database_version(pool: &rocket::State<PgPool>) -> Result<i32, sqlx::Error> {
...
pub async fn mk_lib_database_version_check(pool: &rocket::State<PgPool>,
                                           update_schema: bool) -> Result<bool, sqlx::Error> {
...
#[allow(dead_code)]
pub async fn mk_lib_database_version_update(pool: &rocket::State<PgPool>,
                                            version_number: i32) -> Result<(), sqlx::Error> {
    pool
        .query_as("update mm_version set mm_version_number = $1", &[&version_number])
        .await?;
    Ok(())
}
